...
(function(window, old) {
    ...
    try {
      if (window.Node && obj instanceof window.Node) {
        return (
          ...
          "=" +
          encodeURIComponent(targetToString(obj))
        );
      }
  ...
  function getSetting(name, fallback) {
    ...
    var setting =
      self[name] !== undefined ? self[name] : data[name.toLowerCase()];
    if (setting === "false") {
      setting = false;
    }
  ...
  function sendToBugsnag(details, metaData) {
    ...
    if (payload.lineNumber === 0 && /Script error\.?/.test(payload.message)) {
      return log(
        "Ignoring cross-domain script error. See https://bugsnag.com/docs/notifiers/js/cors"
      );
    }
    ...
    if (!stacktrace) {
      ...
      try {
        ...
        while (curr && functionStack.length < MAX_FAKE_STACK_SIZE) {
          var fn = FUNCTION_REGEX.test(curr.toString())
